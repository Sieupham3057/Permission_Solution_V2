...
public class UserAccountController : BaseApiController
{
	...
	public async Task<IActionResult> GetUserByUserName(string userName)
	{
		var appUser = await _userAccountService.GetUserByUserNameAsync(userName)
			?? throw new UserRoleNotFoundException($"User with name: {userName} not found.");
	...
	[HttpGet("users")]
	[Authorize(AuthPolicies.ViewAllUsersPolicy)] //=> users.view
	...
	[ProducesResponseType(403)]
	[ProducesResponseType(404)]
	public async Task<IActionResult> UpdateUser(Guid id, [FromBody] UserEditVM user)
	{
		var appUser = await _userAccountService.GetUserByIdAsync(id);
		var currentRoles = appUser != null ? (await _userAccountService.GetUserRolesAsync(appUser)).ToArray() : null;
		...
		if (user.Id is not null && id != user.Id)
		...
		// Change Password or UserName yourself need CurrentPassword
		...
		if (ModelState.IsValid)
		{
			...
			if (result.Succeeded)
			{
				...
				{
					if (!string.IsNullOrWhiteSpace(user.CurrentPassword))
						result = await _userAccountService.UpdatePasswordAsync(appUser, user.CurrentPassword, user.NewPassword!);
					else
	...
	[ProducesResponseType(403)]
	[ProducesResponseType(404)]
	public async Task<IActionResult> UpdateUser(Guid id, [FromBody] JsonPatchDocument<UserPatchVM> patch)
	{
		if (!(await _authorizationService.AuthorizeAsync(User, id,
	...
	public async Task<IActionResult> DeleteUser(Guid id)
	{
		...
		{
			AddModelError($"User \"{appUser.UserName}\" cannot be deleted at this time. " + "Delete the associated records and try again.");
		...
		if (ModelState.IsValid)
		{
			...
			if (!result.Succeeded)
				throw new UserAccountException($"The following errors occurred whilst deleting user \"{id}\": " + $"{string.Join(", ", result.Errors)}");
	...
	public async Task<IActionResult> UnblockUser(Guid id)
	{
		...
		if (!result.Succeeded)
			throw new UserAccountException($"The following errors occurred whilst unblocking user: " + $"{string.Join(", ", result.Errors)}");
	...
	public async Task<IActionResult> UserPreferences([FromBody] string? data)
	{
		...
		if (appUser != null)
		{
			...
			if (!result.Succeeded)
				throw new UserAccountException($"The following errors occurred whilst updating User Configurations: " + $"{string.Join(", ", result.Errors)}");
	...
	private async Task<UserVM?> GetUserViewModelHelper(Guid userId)
	{
